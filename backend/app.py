...
def execute_tool(tool, target):
    ...
    commands = {
        ...
        "Gobuster Dir": f"gobuster dir -u https://{target} -w /usr/share/wordlists/common.txt -q --no-error --wildcard",
        "Gobuster DNS": f"gobuster dns --domain {target} -w /usr/share/wordlists/common.txt -q --no-error -t 50",
        "FFUF": f"ffuf -u https://{target}/FUZZ -w /usr/share/wordlists/common.txt -mc 200,301,302,403 -fs 0 -s",
